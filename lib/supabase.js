...
export async function getStats(assignedLeadIds = null) {
  if (assignedLeadIds && assignedLeadIds.length === 0) {
    return { totalLeads: 0, hotLeads: 0, destroyed: 0, pendingDrafts: 0, rebuildValue: 0 }
  }

  let leadsQuery = supabase.from('leads').select('id', { count: 'exact', head: true }).eq('fire_zone_match', true)
  let hotQuery = supabase.from('scores').select('id', { count: 'exact', head: true }).gte('score', 75)
...
export async function getTopLeads(limit = 10, assignedLeadIds = null) {
...
export async function getAllLeads(filters = {}, assignedLeadIds = null) {
  if (assignedLeadIds && assignedLeadIds.length === 0) return []
...
export async function getDrafts(status = null, assignedLeadIds = null) {
  if (assignedLeadIds && assignedLeadIds.length === 0) return []

  let query = supabase.from('drafts').select('*').order('created_at', { ascending: false })
  if (status && status !== 'all') query = query.eq('status', status)
  if (assignedLeadIds) query = query.in('lead_id', assignedLeadIds)
