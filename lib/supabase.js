...
export async function getLeadDetail(id) {
  ...
  // Fetch stacked permits at same address
  let stackedPermits = []
  if (lead.address) {
  ...
  return {
    ...
    // New enrichment fields from owners table
    lot_size_sqft: owner.lot_size_sqft || null,
    owner_occupied: owner.owner_occupied ?? null,
    zoning: owner.zoning || null,
    zoning_desc: owner.zoning_desc || null,
    ...
    // contractor_name, contractor_license, neighbor_permits_500ft, street_permit_count
    // come through ...lead spread from the leads table
    drafts,
    stackedPermits,
    inspections,
  }
