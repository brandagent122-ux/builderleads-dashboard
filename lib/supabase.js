...
export async function getStats(assignedLeadIds = null, market = null, trade = 'gc') {
  ...
  let hotQuery = supabase.from('scores').select('lead_id,score').eq('trade', trade).gte('score', 75)
  let ownersQuery = supabase.from('owners').select('lead_id,assessor_value,skip_trace_data')

  if (assignedLeadIds) {
    let ids = assignedLeadIds
    // If market specified, narrow to assigned leads in that market
    if (market) {
      const { data: marketLeads } = await supabase.from('leads').select('id').eq('market', market).in('id', assignedLeadIds)
      ids = (marketLeads || []).map(l => l.id)
      if (ids.length === 0) {
        return { totalLeads: 0, hotLeads: 0, destroyed: 0, pendingDrafts: 0, rebuildValue: 0 }
      }
    }
    leadsQuery = leadsQuery.in('id', ids)
    hotQuery = hotQuery.in('lead_id', ids)
    ownersQuery = ownersQuery.in('lead_id', ids)
  } else if (market) {
...
export async function getTopLeads(limit = 10, assignedLeadIds = null, market = null, trade = 'gc') {
  if (assignedLeadIds && assignedLeadIds.length === 0) return []

  let filterIds = assignedLeadIds
  if (filterIds && market) {
    // Narrow assigned leads to those in this market
    const { data: marketLeads } = await supabase.from('leads').select('id').eq('market', market).in('id', filterIds)
    filterIds = (marketLeads || []).map(l => l.id)
    if (filterIds.length === 0) return []
  } else if (!filterIds && market) {
    const { data: marketLeads } = await supabase.from('leads').select('id').eq('market', market)
    filterIds = (marketLeads || []).map(l => l.id)
    if (filterIds.length === 0) return []
  }

  let query = supabase.from('scores').select('lead_id,score,reasoning').eq('trade', trade).order('score', { ascending: false })
...
export async function getAllLeads(filters = {}, assignedLeadIds = null, market = null, trade = 'gc') {
  if (assignedLeadIds && assignedLeadIds.length === 0) return []
  ...
  if (assignedLeadIds) {
    ...
    // Also filter by market if specified (client filtering by city)
    if (market) {
  ...
  for (let i = 0; i < leadIds.length; i += batchSize) {
    ...
    const [s, o] = await Promise.all([
      supabase.from('scores').select('lead_id,score').eq('trade', trade).in('lead_id', batch),
...
export async function getLeadDetail(id, assignedLeadIds = null, trade = 'gc') {
  ...
  const [leadRes, scoresRes, ownersRes, draftsRes, inspectionsRes] = await Promise.all([
    ...
    supabase.from('scores').select('*').eq('lead_id', id).eq('trade', trade),
  ...
  return {
    ...
    has_prior_abatement: owner.has_prior_abatement || false,
    has_prior_roofing: owner.has_prior_roofing || false,
    prior_abatement_date: owner.prior_abatement_date || null,
    prior_roofing_date: owner.prior_roofing_date || null,
    total_historical_permits: owner.total_historical_permits || 0,
    drafts,
    stackedPermits,
    inspections,
  }
