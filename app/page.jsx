...
export default function CommandCenter() {
  ...
  async function loadData() {
    ...
    // Read trade from profile
    const trade = ctx.profile?.trade || 'gc'
    setTradeConfig(getTradeConfig(trade))
  ...
  const tc = tradeConfig

  // Build KPI values from stats using trade config keys
  const kpiValues = tc.kpiKeys.map(key => {
    const val = stats[key]
    if (key === 'rebuildValue') return formatMoney(val || 0)
    if (key === 'newThisWeek') return leads.length.toString()
    return (val || 0).toLocaleString()
  })

  const kpiSubValues = tc.kpiSubs.map((sub, i) => {
    if (sub) return sub
    const key = tc.kpiKeys[i]
    if (key === 'hotLeads') return `${stats.totalLeads > 0 ? Math.round(stats.hotLeads/stats.totalLeads*100) : 0}%`
    if (key === 'destroyed') return `${stats.totalLeads > 0 ? Math.round(stats.destroyed/stats.totalLeads*100) : 0}%`
    return ''
  })
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="font-mono text-[11px] text-ink-3 mt-2 tracking-wider uppercase">
            {tc.briefLabel} &middot; {today}
          </p>
        </div>
        {newCount > 0 && (
          <div className="font-mono text-[11px] px-3 py-1.5 rounded-xl border"
            style={{ color: tc.color, background: `rgba(${tc.colorRgb}, 0.08)`, borderColor: `rgba(${tc.colorRgb}, 0.2)` }}>
      ...
      {showBrief && (
        <div className="atlas-card p-4 mb-6 flex items-start gap-4">
          <div className="flex items-center gap-2 flex-shrink-0 pt-0.5">
            <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: tc.color }} />
            <span className="font-mono text-[11px] tracking-wider" style={{ color: tc.color }}>UPDATE</span>
          </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-semibold text-ink-0">{tc.topLeadsLabel}</h2>
          <span className="font-mono text-[11px] px-2.5 py-1 rounded-lg"
            style={{ color: tc.color, background: `rgba(${tc.colorRgb}, 0.08)` }}>
            TOP {leads.length}
          </span>
        </div>
      ...
      <div className="flex flex-col gap-3">
        {leads.map((lead, i) => (
          <LeadCard key={lead.id} lead={lead} rank={i + 1} isNew={isNew(lead)} tradeConfig={tc}
            onClick={() => { markSeen(lead.id); router.push(`/leads/${lead.id}`) }} />
        ))}
      </div>

      <div className="grid grid-cols-2 gap-4 mt-8">
        {tc.quickActions.map((action, i) => (
          <a key={i} href={action.href} className="card-raised p-5 flex items-center gap-4 card-hover cursor-pointer">
            ...
            <div>
              <div className="text-[15px] font-medium text-ink-0">{action.label}</div>
              <div className="font-mono text-[11px] text-ink-3 mt-0.5">
                {action.sub === 'DRAFTS PENDING' ? `${stats.pendingDrafts} ${action.sub}` : `${stats.totalLeads} ${action.sub}`}
              </div>
            </div>
          </a>
        ))}
...
function KPITile({ icon, label, value, sub, accent, tradeColor }) {
  const icons = {
  ...
  return (
    <div className="card-raised-lg p-5">
      <div className={`icon-chip ${accent ? '' : 'icon-chip-sunk'} mb-3`}
        style={accent ? { background: tradeColor, color: 'var(--page)' } : { color: tradeColor }}>
        {icons[icon]}
      </div>
      <div className="font-mono text-[11px] tracking-wider uppercase" style={{ color: '#8B8B96' }}>{label}</div>
      <div className="text-[38px] font-bold leading-none mt-1 tracking-tight"
        style={{ color: accent ? tradeColor : 'var(--ink-0)' }}>{value}</div>
      <div className="font-mono text-[11px] text-ink-3 mt-1">{sub}</div>
    </div>
  )
}

function LeadCard({ lead, rank, isNew, onClick, tradeConfig }) {
  const tc = tradeConfig
  const scoreColor = lead.score >= 85 ? tc.color : lead.score >= 75 ? tc.color : lead.score >= 50 ? 'var(--amber)' : 'var(--ink-3)'
    ...
    <div onClick={onClick} className={`card-raised card-hover p-4 flex items-center gap-4 cursor-pointer ${isNew ? 'lead-new' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-1.5 mt-2 flex-wrap">
          ...
          {tc.slug === 'enviro' && lead.year_built > 0 && lead.year_built < 1978 && (
            <span className="tag" style={{ background: 'rgba(255,68,68,0.12)', color: '#FF4444' }}>PRE-1978</span>
          )}
          {tc.slug === 'enviro' && lead.year_built > 0 && lead.year_built < 1978 && (
            <span className="tag" style={{ background: 'rgba(255,68,68,0.08)', color: '#FF6666' }}>HIGH RISK</span>
          )}
        </div>
      </div>
